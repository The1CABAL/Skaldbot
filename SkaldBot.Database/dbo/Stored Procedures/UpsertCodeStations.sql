...
-- =============================================
CREATE PROCEDURE UpsertCodeStations
	@id								int,
	@name							nvarchar(max),
...
BEGIN
	...
	BEGIN
		INSERT INTO CodeStations (id, [name], system_id, updated_at, max_landing_pad, distance_to_star, government_id, government, allegiance_id, allegiance, states, [type_id], [type], has_blackmarket, has_market, has_refuel, has_repair, has_rearm, has_shipyard, has_docking, has_commodities, import_commodities, export_commodities,	prohibited_commodities, economies, shipyard_updated_at, outfitting_updated_at, market_updated_at, is_planetary, selling_ships, selling_modules, settlement_size_id, settlement_size, settlement_security_id, settlement_security, body_id, controlling_minor_faction_id, ed_market_id)
		VALUES (@id, @name,	@system_id,	@updated_at, @max_landing_pad, @distance_to_star, @government_id, @government, @allegiance_id, @allegiance,	@states, @type_id, @type, @has_blackmarket, @has_market, @has_refuel, @has_repair, @has_rearm, @has_shipyard, @has_docking, @has_commodities, @import_commodities, @export_commodities,	@prohibited_commodities, @economies, @shipyard_updated_at, @outfitting_updated_at, @market_updated_at, @is_planetary, @selling_ships, @selling_modules, @settlement_size_id, @settlement_size, @settlement_security_id,	@settlement_security, @body_id,	@controlling_minor_faction_id, @ed_market_id)
	END
	ELSE
	BEGIN
		UPDATE CodeStations SET [name] = @name, system_id = @system_id, updated_at = @updated_at, max_landing_pad = @max_landing_pad, distance_to_star = @distance_to_star, government_id = @government_id, government = @government, allegiance_id = @allegiance_id, allegiance = @allegiance, states = @states, [type_id] = @type_id, [type] = @type, has_blackmarket = @has_blackmarket, has_market = @has_market, has_refuel = @has_refuel, has_repair = @has_repair, has_rearm = @has_rearm, has_shipyard = @has_shipyard, has_docking = @has_docking, has_commodities = @has_commodities, 
